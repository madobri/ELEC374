...
module DataPath (
    ...
    output wire HIin, LOin, MARin, MDRin, Yin, Zin, PCin, IRin,
    output wire HIout, LOout, MARout, MDRout, PCout, Cout, Inout, Outout,
    output wire IncPC, wren, muxSignal, zhighout, zlowout,
    output wire Gra, Grb, Grc, Rin, Rout, BAout, inport_in, outport_in,
    output wire con_in, read, clear,
	 
	 output wire [31:0] encode, 
    ...
    .PCin(PCin), .IRin(IRin), .HIout(HIout), .LOout(LOout), .MARout(MARout), 
    .MDRout(MDRout), .PCout(PCout), .Cout(Cout), .Inout(Inout), .Outout(Outout), 
    .IncPC(IncPC), .wren(wren), .muxSignal(muxSignal), .zhighout(zhighout), 
    .zlowout(zlowout), .Gra(Gra), .Grb(Grb), .Grc(Grc), .Rin(Rin), .Rout(Rout), 
    .BAout(BAout), .inport_in(inport_in), .outport_in(outport_in), 
    ...
    register InPort (clear, clk, inport_in, fromInPort, InPort_data);
    ...
    MAR_register MAR (.clear(clear), .clk(clk), .mar_in(MARin), .bus_data(BusLine), .mar_address_out(MAR_address));
	 ...
	 PC_register PC (clear, clk, (PCin | con_out), IncPC, BusLine, PC_data);
    ...
    ALU alu(.clk(clk), .signal(opcode), .A(YLine), .B(BusLine), .result(ZLine));
	 ...
	 assign encode = {8'b0, Cout,  Inout, MDRout, PCout, zlowout, zhighout, LOout, HIout, Result_out};
    ...
    // Memory
    RAM ram(.index(MAR_address), .clock(clk), .w_data(MDR_data), .enable(wren), .r_data(Ram_data));
